...
const ModelTraining = () => {
  ...
  const handleCreate = async () => {
    const formData = {
      ...
      framework_option: '', // optional or extend if needed
    };
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 4 }}>
        <Grid container spacing={3}>
          <Grid item xs={12} sm={6} md={3}>
            <TextField label="Env Name" fullWidth value={envName} onChange={(e) => setEnvName(e.target.value)} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <TextField label="Env Purpose" fullWidth value={envPurpose} onChange={(e) => setEnvPurpose(e.target.value)} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
